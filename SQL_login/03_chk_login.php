...
// 資料撈出來後做比對  $acc和$pw 等於 $user提供的acc與pw資訊 相同時 執行if
        // if($acc==$user['acc'] && $pw==$user['pw']){
            // header("location:04_member_center.php");// 正確 導到會員中心頁面
        // }else{
            // header("location:01_login.php?error=帳號或密碼錯誤");// 錯誤 回到登入頁面
        // }
?>
<!-- ------------------------------------------作法二.僅計算有無登入成功------------------------------------------ -->
<!-- 需求 : 簡易的純登入系統 -->
...
if($chk){ // $_SESSION['user']=$acc; 使用這個方法 要注意 帳號的唯一性 避免系統碰到相同帳號時無法處理 發生錯誤
    ...
    header("location:04_member_center.php");
}else{
    header("location:01_login.php?error=帳號或密碼錯誤");
}
?>
