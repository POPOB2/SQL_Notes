...
<!-- -------------------------------------------------將表單收到的資料存入資料庫,並將更新的資料呈現在做好的資料表------------------------------------------------- -->
...
<?php
        $dsn = "mysql:host=localhost;charset=utf8;dbname=school"; // 建立參數設定檔
        $pdo = new PDO($dsn,'root',''); // 建立PDO的連線(1.資料庫的設定,2.使用者名稱,3.使用者密碼)

// 一般為了預防發生46行的註解內容, 會將表單內容加工處理 
...
// 收到新增的表單資料時, 直接送進資料庫
        $pdo->query($sql);//讓pdo使用query功能 執行$sql
